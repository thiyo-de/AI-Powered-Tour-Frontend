(function () {
    ...
    const TDVBridge = {
        ...
        navigateByLabel: function (label) {
            ...
            const exactMatch = this._panoramas.find(p => p.label.toLowerCase() === label.toLowerCase().trim());
            if (exactMatch) {
            ...
            if (window.Fuse && this._panoramas.length > 0) {
                ...
                if (result.length > 0) {
                    console.log('[TDVBridge] Fuzzy Navigating to:', result[0].item.label, '(index:', result[0].item.playlistIndex + ')');
        ...
        navigateLast: function() {
            if (this._panoramas.length > 0) {
                window.tour.setMediaByIndex(this._panoramas[this._panoramas.length - 1].playlistIndex);
                return true;
            }
            return false;
        },
        navigateNext: function() {
            ...
            const currentLabel = this.getCurrentLocation();
            const currentIndex = this._panoramas.findIndex(p => p.label === currentLabel);
            if (currentIndex >= 0 && currentIndex < this._panoramas.length - 1) {
                window.tour.setMediaByIndex(this._panoramas[currentIndex + 1].playlistIndex);
                return true;
            }
        ...
        isMenuOpen: () => typeof window.isMenuOpen === 'function' && window.isMenuOpen(),
        openMenu: () => typeof window.openMenu === 'function' && window.openMenu(),
        closeMenu: () => typeof window.closeMenu === 'function' && window.closeMenu(),

        isSearchOpen: () => {
            const searchBox = document.getElementById('container_search');
            return searchBox && searchBox.style.display !== 'none';
        },
        openSearch: function () {
            if (!this.isSearchOpen() && typeof window.toggleSearchBox === 'function') window.toggleSearchBox();
        },
        closeSearch: function () {
            if (this.isSearchOpen() && typeof window.toggleSearchBox === 'function') window.toggleSearchBox();
        },

        isContactOpen: () => typeof window.isContactOpen === 'function' && window.isContactOpen(),
        openContact: () => typeof window.openContact === 'function' && window.openContact(),
        closeContact: () => typeof window.closeContact === 'function' && window.closeContact(),
        ...
        openPanoramaList: function () {
            if (!this.isPanoramaListOpen() && typeof window.togglePanoList === 'function') window.togglePanoList();
        },
        closePanoramaList: function () {
            if (this.isPanoramaListOpen() && typeof window.togglePanoList === 'function') window.togglePanoList();
        },
        openStreetView: () => typeof window.openGsvPopup === 'function' && window.openGsvPopup(),
    ...
    const LocalIntentRouter = {
        ...
        init: function () {
            const intents = [
                { action: 'start_guided_tour', phrases: ['start tour', 'begin tour', 'give me a tour', 'show me around', 'launch tour', 'campus tour', 'guided tour', 'give me a campus tour', 'start guided tour', 'start tour again', 'resume tour'] },
                { action: 'stop_guided_tour', phrases: ['stop tour', 'end tour', 'cancel tour', 'quit tour', 'exit tour'] },
                { action: 'next_tour_stop', phrases: ['next', 'continue', 'go', 'proceed', 'ok', 'yes', 'next stop', 'next room'] },
                { action: 'previous_tour_stop', phrases: ['previous stop', 'go back a stop', 'last stop'] },
                { action: 'get_current_location', phrases: ['where am i', 'what is this place', 'where are we', 'location'] },
                { action: 'describe_current_location', phrases: ['tell me about this place', 'tell about', 'describe this place', 'what is here', 'what is this room', 'information', 'more info'] },
                { action: 'toggle_fullscreen', phrases: ['fullscreen', 'full screen', 'maximize', 'make it bigger', 'expand', 'exit fullscreen', 'close fullscreen'] },
                { action: 'toggle_music', phrases: ['music', 'play music', 'stop music', 'mute', 'unmute', 'sound', 'audio'] },
                { action: 'set_music_volume', phrases: ['volume', 'turn it up', 'turn it down', 'louder', 'quieter'] },
                { action: 'open_menu', phrases: ['menu', 'open menu', 'show menu'] },
                { action: 'close_menu', phrases: ['close menu', 'hide menu'] },
                { action: 'open_panorama_list', phrases: ['panorama list', 'panoramas', 'show list', 'locations list', 'facilities', 'show me the facilities'] },
                { action: 'close_panorama_list', phrases: ['close panorama list', 'hide list'] },
                { action: 'open_search', phrases: ['search', 'find', 'looking for'] },
                { action: 'close_search', phrases: ['close search', 'hide search'] },
                { action: 'open_contact', phrases: ['contact us', 'contact', 'get in touch', 'phone number', 'email'] },
                { action: 'close_contact', phrases: ['close contact', 'hide contact'] },
                { action: 'open_street_view', phrases: ['street view', 'google street view', 'outside'] },
                { action: 'close_street_view', phrases: ['close street view', 'hide street view'] },
                { action: 'navigate_previous', phrases: ['go back', 'back', 'previous', 'return', 'previous pano', 'go back pano', 'go prious pano'] },
                { action: 'navigate_first', phrases: ['start', 'entrance', 'home', 'beginning', 'go to start', 'go first pano', 'go to the first pano', 'first panorama', 'start from the beginning'] },
                { action: 'navigate_last', phrases: ['go last pano', 'last panorama', 'go to the last panorama', 'skip to the end'] },
                { action: 'navigate_next', phrases: ['next pano', 'go next pano', 'move forward'] },
                { action: 'random_panorama', phrases: ['random', 'surprise me', 'take me somewhere new', 'anywhere'] },
                { action: 'look_around', phrases: ['look around', 'rotate', 'spin', 'show me the room'] },
                { action: 'reset_view', phrases: ['reset view', 'center', 'look forward'] },
                { action: 'zoom_in', phrases: ['zoom in', 'closer', 'magnify'] },
                { action: 'zoom_out', phrases: ['zoom out', 'farther', 'further'] },
                { action: 'open_related_campus', phrases: ['other campus', 'related campus', 'visit another campus'] },
                // Chit-Chat
                { action: 'chitchat_hello', phrases: ['hello', 'hi', 'hey', 'greetings', 'good morning', 'good afternoon'] },
                { action: 'chitchat_how_are_you', phrases: ['how are you', 'how are you doing', 'whats up'] },
                { action: 'chitchat_who_are_you', phrases: ['who are you', 'what are you', 'who made you', 'who built you'] },
                { action: 'chitchat_help', phrases: ['help', 'what can you do', 'how does this work', 'instructions'] }
            ];
        ...
        parse: function (text) {
            ...
            const numberPattern = /(?:go(?: to)? |take me to |show me )?(?:the )?(1?\d|20|first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth)(?:st|nd|rd|th)?\s+(?:pano|panorama|stop|room|location)/i;
            const alternatePattern = /(?:pano|panorama|stop|room|location)\s+(1?\d|20|first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth)/i;
            const indexMatch = lowerText.match(numberPattern) || lowerText.match(alternatePattern);
            ...
            if (indexMatch) {
                ...
                if (!isNaN(panoNumber)) {
                    let functionCall = { name: 'navigate_to_index', args: { index: panoNumber - 1 } }; // 0-based
                    return {
                        text: ChatManager._getFallbackText([functionCall]),
                        function_calls: [functionCall],
                        suggestions: ChatManager.getContextualSuggestions(TDVBridge.getCurrentLocation(), 'navigate_to_index')
                    };
                }
            }
            ...
            if (result.length > 0 && result[0].score < 0.5) { // 0.5 threshold to capture components and settings
                ...
                if (action.startsWith('chitchat_')) {
                    ...
                    if (action === 'chitchat_hello') {
                        const hellos = ["Hello there! Ready to explore the campus?", "Hi! How can I help you today?", "Greetings! Where would you like to go?"];
                        responseText = hellos[Math.floor(Math.random() * hellos.length)];
                    } else if (action === 'chitchat_how_are_you') {
                        responseText = "I'm doing great, thanks for asking! Ready to guide you around.";
                    } else if (action === 'chitchat_who_are_you') {
                        responseText = "I am your virtual campus guide. I can help you navigate the school, control the tour, and show you around.";
                    } else if (action === 'chitchat_help') {
                        responseText = "You can ask me to 'Start a guided tour', 'Go to the Library', 'Zoom in', 'Open the menu', or 'Look around'. What would you like to do?";
                    }
                    return {
                        text: responseText,
                        function_calls: [],
                        suggestions: ChatManager.getContextualSuggestions(TDVBridge.getCurrentLocation(), action)
                    };
                }
                ...
                if (action === 'zoom_in') functionCall = { name: 'zoom_camera', args: { direction: 'in', amount: 20 } };
                if (action === 'zoom_out') functionCall = { name: 'zoom_camera', args: { direction: 'out', amount: 20 } };

                // Map specific volume arguments
                if (action === 'set_music_volume') {
                    if (lowerText.includes('down') || lowerText.includes('quiet')) functionCall.args = { level: 20 };
                    else if (lowerText.includes('up') || lowerText.includes('loud')) functionCall.args = { level: 80 };
            ...
            if (TDVBridge._panoramas.length > 0) {
                ...
                let query = lowerText.replace(/\b(go to|go|take me to|take me|show me|show|navigate to|navigate|where is|where|open|the)\b/g, '').replace(/\s+/g, ' ').trim();
                ...
                if (panoMatch.length > 0 && panoMatch[0].score < 0.25) {
                    ...
                    if (targetLocation === currentLocation) {
                        return {
                            ...
                            function_calls: [],
                            suggestions: ChatManager.getContextualSuggestions(currentLocation, 'navigate_to_panorama')
                        };
                    } else {
                        return {
                            text: `Taking you to ${targetLocation} now!`,
                            function_calls: [{ name: 'navigate_to_panorama', args: { location: targetLocation } }],
                            suggestions: ChatManager.getContextualSuggestions(targetLocation, 'navigate_to_panorama')
                        };
                    }
                } else if (lowerText.match(/\b(go to|go|take me to|take me|show me|show|navigate to|navigate|where is|where|open)\b/)) {
                    ...
                    return {
                        ...
                        function_calls: [],
                        suggestions: TDVBridge._panoramas.map(p => p.label.trim()),
                        error: false
                    };
                }
            }
            ...
            return {
                ...
                function_calls: [],
                suggestions: ChatManager.getContextualSuggestions(TDVBridge.getCurrentLocation(), 'fallback'),
                error: false
            };
        }
    };
    ...
    const ChatManager = {
        ...
        _getFallbackText: function (calls) {
            ...
            const fallbacks = {
                get_current_location: () => {
                    ...
                    return v([`You are currently at ${loc}.`, `This is ${loc}.`, `We are at ${loc} right now.`]);
                },
                describe_current_location: () => {
                    const currentLocation = TDVBridge.getCurrentLocation().trim();
                    const panos = TDVBridge._panoramas;
                    const idx = panos.findIndex(p => p.label.trim() === currentLocation);
                    if (idx >= 0 && TourManager._about_narrations[idx]) {
                        return TourManager._about_narrations[idx];
                    }
                    return v([`This is the ${currentLocation}. Feel free to look around.`, `We are currently at the ${currentLocation}.`]);
                },
                navigate_to_panorama: (a) => v([`Taking you to ${a.location || 'that location'} now!`, `Heading over to ${a.location || 'that spot'}.`, `Let's go to ${a.location || 'there'}!`]),
                control_camera: (a) => v([`Looking ${a.direction}.`, `Turning ${a.direction}.`]),
                zoom_camera: (a) => v([`Zooming ${a.direction}.`, `Getting a closer look.`]),
                toggle_music: () => v(['Toggling the background music.', 'Adjusting the music.']),
                toggle_fullscreen: () => v(['Switching fullscreen mode.', 'Changing view mode.']),
                look_around: () => v(["Let's take a look around!", "Scanning the room."]),
                stop_guided_tour: () => v(['Tour stopped. Let me know if you need anything.', 'Tour cancelled.']),
                next_tour_stop: () => v(['Moving to the next stop!', 'Onward to the next location.']),
                previous_tour_stop: () => v(['Going back to the previous stop!', 'Returning to the last stop.']),
                jump_to_tour_stop: (a) => v([`Jumping to ${a.location || 'that stop'} now!`, `Skipping to ${a.location}.`]),
                open_panorama_list: () => TDVBridge.isPanoramaListOpen() ? "The location list is already visible." : v(['Opening the panorama list.', 'Here are the locations.']),
                close_panorama_list: () => !TDVBridge.isPanoramaListOpen() ? "The location list is not open." : v(['Closing the panorama list.', 'List hidden.']),
                open_search: (a) => TDVBridge.isSearchOpen() ? "Search is already open." : (a.query ? `Searching for "${a.query}"...` : v(['Opening search.', 'Let\'s find something.'])),
                close_search: () => !TDVBridge.isSearchOpen() ? "Search is not currently open." : v(['Closing search.', 'Search hidden.']),
                open_contact: () => TDVBridge.isContactOpen() ? "The contact info is already on screen." : v(['Opening the contact information.', 'Here are our contact details.']),
                close_contact: () => !TDVBridge.isContactOpen() ? "The contact info is not open." : v(['Closing the contact panel.', 'Contact info hidden.']),
                open_menu: () => TDVBridge.isMenuOpen() ? "The menu is already open." : v(['Opening the menu.', 'Here is the menu.']),
                close_menu: () => !TDVBridge.isMenuOpen() ? "The menu is already closed." : v(['Closing the menu.', 'Menu hidden.']),
                open_street_view: () => v(['Opening Google Street View.', 'Taking you outside.']),
                close_street_view: () => v(['Closing Google Street View.', 'Returning inside.']),
                navigate_previous: () => {
                    const c = TDVBridge.getCurrentLocation();
                    const i = TDVBridge._panoramas.findIndex(p => p.label === c);
                    return i > 0 ? v(['Going back to the previous location.', 'Returning to where we were.']) : "You are currently at the first panorama, so we can't go back further.";
                },
                navigate_first: () => {
                    const c = TDVBridge.getCurrentLocation();
                    const i = TDVBridge._panoramas.findIndex(p => p.label === c);
                    return i > 0 ? v(['Heading back to the entrance.', 'Going to the start.']) : "You are already at the first panorama.";
                },
                navigate_last: () => {
                    const c = TDVBridge.getCurrentLocation();
                    const i = TDVBridge._panoramas.findIndex(p => p.label === c);
                    return (i >= 0 && i < TDVBridge._panoramas.length - 1) ? v(['Taking you to the last panorama.', 'Skipping to the end.']) : "You are already at the very last panorama.";
                },
                navigate_next: () => {
                    const c = TDVBridge.getCurrentLocation();
                    const i = TDVBridge._panoramas.findIndex(p => p.label === c);
                    return (i >= 0 && i < TDVBridge._panoramas.length - 1) ? v(['Taking you to the next panorama now.', 'Moving to the next location.']) : "You are already at the very last panorama in the tour.";
                },
                navigate_to_index: (a) => {
                    if (a.index >= 0 && a.index < TDVBridge._panoramas.length) {
                        return v([`Taking you to panorama ${a.index + 1}.`, `Moving to location ${a.index + 1}.`]);
                    }
                    return "That panorama number does not exist in this tour.";
                },
                random_panorama: () => v(['Taking you somewhere new — enjoy the surprise!', 'Let\'s explore a random spot.']),
                reset_view: () => v(['Camera reset to the default view.', 'View centered.']),
        ...
        getContextualSuggestions: function (locationName, lastAction) {
            ...
            if (TourManager && TourManager.activePlan && TourManager._isPaused) {
                return ["Resume tour", "Stop tour"];
            }
            if (lastAction === 'toggle_music' || lastAction === 'set_music_volume') {
                return ["Play music", "Next room", "Look around"];
            }
            if (lastAction === 'open_menu') return ["Close menu", "Contact us", "Search"];
            if (lastAction === 'open_search') return ["Close search", "Show menu"];

            // Location-based
            const loc = (locationName || "").toLowerCase();
            if (loc.includes('library')) return ["Go to chemistry lab"];
            if (loc.includes('lab')) return ["Go to library", "Go to classroom"];
            if (loc.includes('sports') || loc.includes('ground')) return ["Go to main building", "Start tour"];
            if (loc.includes('entrance')) return ["Start guided tour", "Go to courtyard", "Open menu"];

            return ["Where am I?", "Next stop"];
        },
        ...
        displayResponse: function (response, tourWasInterrupted) {
            ...
            if (response.suggestions && response.suggestions.includes("Resume tour") && (!TourManager || !TourManager.activePlan)) {
                let primaryAction = response.function_calls && response.function_calls.length > 0 ? response.function_calls[0].name : '';
                response.suggestions = this.getContextualSuggestions(TDVBridge.getCurrentLocation(), primaryAction);
            }
                ...
                getBlob
                    .then(blob => {
                        ...
                        const updateHighlight = () => {
                            if (!self._currentAudio || !groqWordMap.length) return;
                            ...
                            if (dur && !isNaN(dur)) {
                                ...
                                const estimatedChar = Math.floor(progress * sentence.length);
        ...
        execute: function (calls) {
            for (const call of calls) {
                ...
                try {
                    ...
                    const allowedDuringTour = ['stop_guided_tour', 'next_tour_stop', 'previous_tour_stop', 'look_around', 'toggle_fullscreen', 'toggle_music', 'set_music_volume'];
                    if (TourManager && TourManager.activePlan && !allowedDuringTour.includes(call.name)) {
                        console.log(`[FunctionExecutor] Cancelling active tour due to conflicting request: ${call.name}`);
                    ...
                    switch (call.name) {
                        ...
                        case 'navigate_to_index':
                            if (call.args && call.args.index >= 0 && call.args.index < TDVBridge._panoramas.length) {
                                window.tour.setMediaByIndex(TDVBridge._panoramas[call.args.index].playlistIndex);
                            }
                            break;
    ...
    const TourManager = {
        ...
        advanceTimeout: null,
        _narrations: {
            '0': "Welcome to Mount Zion International School! Let's begin our tour right here, at the Main Entrance. Every single morning, over a thousand students walk through these gates to start their learning journey. Notice the warm and welcoming design. It really sets the perfect tone for everything you're about to see inside.",
            '1': "Here we are, at the Sports Facility! This sprawling ground comes alive with energy every single day. From football matches to athletic training, this is exactly where our students build teamwork, discipline, and physical fitness.",
            '2': "This is the front entrance of our main academic building. The architecture beautifully blends modern design with practical, student-friendly spaces. Right behind this facade are the major classrooms and offices that make up the heart of our academic life.",
            '3': "Welcome to the School Courtyard. You could say this is truly the soul of campus life! Every morning, the entire school gathers right here for assembly. It's a vibrant open space where cultural programs, celebrations, and even spontaneous games take place.",
            '4': "Let's step into the Computer Lab. It's a state-of-the-art facility, equipped with modern systems and high-speed internet. In fact, coding is taught here from as early as Grade Three! It truly reflects our commitment to preparing students for a technology-driven world.",
            '5': "Welcome to the Chemistry Lab, where science really comes to life! Following strict safety standards, students perform dozens of hands-on experiments here every year. We have individual workstations, a fume hood, and fully stocked equipment benches. Learning by doing is our motto!",
            '6': "Ah, the Library. It's a quiet sanctuary housing over five thousand books, spanning every subject and reading level. There's even a dedicated children's section for our youngest readers. This space is perfectly designed to nurture a lifelong love of learning.",
            '7': "And finally, here is our Kindergarten Classroom! This is a colorful and joyful space, designed specifically for the youngest minds on campus. We focus heavily on play-based learning. With interactive boards, art supplies, and learning aids, we make sure every day is an adventure!"
        },
        _about_narrations: {
            '0': "This is the Main Entrance of Mount Zion International School. Every morning, over a thousand students walk through these gates to start their learning journey. The warm and welcoming design sets the perfect tone for the campus.",
            '1': "This is the Sports Facility. This sprawling ground comes alive with energy every single day. From football matches to athletic training, this is where our students build teamwork, discipline, and physical fitness.",
            '2': "This is the front entrance of the main academic building. The architecture blends modern design with practical, student-friendly spaces. Right behind this facade are the major classrooms and offices.",
            '3': "This is the School Courtyard, the soul of campus life. Every morning, the entire school gathers here for assembly. It's a vibrant open space where cultural programs, celebrations, and spontaneous games take place.",
            '4': "This is the Computer Lab, a state-of-the-art facility equipped with modern systems and high-speed internet. Coding is taught here from as early as Grade Three, reflecting a commitment to preparing students for a technology-driven world.",
            '5': "This is the Chemistry Lab. Following strict safety standards, students perform dozens of hands-on experiments here every year. We have individual workstations, a fume hood, and fully stocked equipment benches.",
            '6': "This is the Library, a quiet sanctuary housing over five thousand books spanning every subject and reading level. There is even a dedicated children's section, designed to nurture a lifelong love of learning.",
            '7': "This is the Kindergarten Classroom, a colorful and joyful space designed specifically for the youngest minds on campus. We focus heavily on play-based learning with interactive boards, art supplies, and learning aids."
        },
        _isPaused: false,
            ...
            return new Promise(function (resolve) {
                ...
                if (!VoiceManager._useBrowserTTS) {
                    var check = setInterval(function () {
                        ...
                        var playing = (VoiceManager._currentAudio && !VoiceManager._currentAudio.paused) || VoiceManager._isFetchingGroq;
